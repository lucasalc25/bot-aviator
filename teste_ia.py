...
def capturar_tela(chave, x, y, xfinal, yfinal):
    ...
    if chave == 1:
        ...
        image = cv2.imread('screenshot_numeros.png')
    if chave == 2:
        image.save("screenshot_novo_numero.png")
        # Carregar a imagem da tela capturada
        image = cv2.imread('screenshot_novo_numero.png')
    if chave == 3:
        image.save("screenshot_saldo.png")
        # Carregar a imagem da tela capturada
        image = cv2.imread('screenshot_saldo.png')
    if chave == 4:
        image.save("screenshot_botao.png")
        # Carregar a imagem da tela capturada
        image = cv2.imread('screenshot_botao.png')
    if chave == 5:
        image.save("screenshot_win.png")
        # Carregar a imagem da tela capturada
        image = cv2.imread('screenshot_win.png')
...
def capturar_numeros():
    image = capturar_tela(1, 590, 240, 1760, 325)
...
def capturar_novo_numero():
    image = capturar_tela(2, 600, 195, 665, 225)
...
def main():
    ...
    while True:
        novo_numero = lista_numeros[0]
        previsao = fazer_previsao(modelo, novo_numero)
        print(f"Previsão: {previsao}")
        
        if previsao >= 1.25:
            resposta = input("O palpite está correto? (s/n): ")
            if resposta.lower() == 's':
                continue
            else:
                for _ in range(3):
